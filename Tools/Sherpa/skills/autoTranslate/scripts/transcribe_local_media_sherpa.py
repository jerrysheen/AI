...
def ensure_wav_input(input_path: Path, staging_dir: Path, ffmpeg_path: str | None) -> tuple[Path, dict]:
    ...
    cmd = [ffmpeg, "-y", "-i", str(input_path), "-ac", "1", "-ar", "16000", str(output_path)]
...
def build_srt(result: dict) -> str:
    ...
    for idx, token in enumerate(tokens):
        ...
        token_text = token.strip() or token
...
def main() -> int:
    ...
    provider = args.provider or env.get("AI_AUTO_TRANSLATE_SHERPA_PROVIDER", "cuda")
    ...
    print(f"[sherpa] input={input_path}")
    print(f"[sherpa] wav={wav_path}")
    print(f"[sherpa] provider={provider}")
    print(f"[sherpa] model_dir={model_dir}")
