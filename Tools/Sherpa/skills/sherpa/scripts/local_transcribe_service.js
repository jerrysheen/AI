...
async function launchSherpaJob(payload) {
  ...
  const job = {
    ...
    provider: payload.provider || env.AI_AUTO_TRANSLATE_SHERPA_PROVIDER || "cpu",
  ...
  const child = spawn(pythonExe, args, { cwd: repoRoot(), env: process.env, stdio: ["ignore", "pipe", "pipe"] });
  job.status = "running";
  job.pid = child.pid;
  let stdout = "";
  let stderr = "";
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") {
      return sendJson(res, 200, {
        ...
        supportedBackends: ["sherpa"],
        provider: env.AI_AUTO_TRANSLATE_SHERPA_PROVIDER || "cpu",
        runsRoot,
      });
    }
    if (req.method === "POST" && url.pathname === "/jobs") {
      const body = await readJson(req);
      if (!body.mediaPath) return sendJson(res, 400, { ok: false, error: "mediaPath is required" });
      if (body.backend && body.backend !== "sherpa") return sendJson(res, 400, { ok: false, error: `Unsupported backend: ${body.backend}` });
      const job = await launchSherpaJob(body);
      return sendJson(res, 202, { ok: true, job });
    }
    if (req.method === "GET" && url.pathname.startsWith("/jobs/")) {
      const jobId = url.pathname.split("/").pop();
      if (!jobs.has(jobId)) return sendJson(res, 404, { ok: false, error: "Job not found" });
...
ensureDir(runsRoot).then(() => {
  server.listen(port, host, () => {
    console.log(`[sherpa-service] listening on http://${host}:${port}`);
  });
});
