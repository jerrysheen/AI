...
def main() -> int:
    ...
    report = {"repo_root": str(root), "provider": provider, "checks": []}
    ...
    try:
        ...
        recognizer = sherpa_onnx.OfflineRecognizer.from_sense_voice(model=str(model_path), tokens=str(tokens_path), language=language, use_itn=use_itn, provider=provider, num_threads=num_threads, debug=False)
        report["checks"].append({"name": "recognizer_create", "ok": True, "detail": f"{time.perf_counter() - load_start:.3f}s"})
