...
async function ensureBilibiliBrowser(configPath) {
  const scriptDir = __dirname;
  const skillDir = path.resolve(scriptDir, "..");
  const repoRoot = path.resolve(skillDir, "..", "..");
  const resolvedConfigPath = path.resolve(configPath || path.join(skillDir, "config", "bilibili-browser.json"));
  ...
  const startupUrl = String(config?.site?.startup_url || config?.site?.url || "https://www.bilibili.com");
...
async function main() {
  try {
    const port = await ensureBilibiliBrowser(process.argv[2]);
