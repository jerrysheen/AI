...
- `pull-tiktok` - (In progress) TikTok video downloader
...
## pull-tiktok Specific Guidance

This skill is currently in development. See `Task.md` for the current task definition.

Goal: Implement a TikTok video downloader that accepts share links/text, extracts the video, downloads to local file, and returns path + metadata.

Acceptance criteria (from Task.md):
- Input normalization (extract link from share text)
- Real video file downloaded locally
- Local file path returned
- File exists and size recorded
- Stable filename
- Result includes: source_url, resolved_url, local_path, file_size, optional metadata
