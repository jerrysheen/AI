...
async function ensureTwitterBrowser(configPath) {
  const scriptDir = __dirname;
  const skillDir = path.resolve(scriptDir, "..");
  const repoRoot = path.resolve(skillDir, "..", "..");
  const resolvedConfigPath = path.resolve(configPath || path.join(skillDir, "config", "twitter-browser.json"));
  ...
  const startupUrl = String(config?.site?.startup_url || config?.site?.url || "https://twitter.com");
...
async function main() {
  try {
    const port = await ensureTwitterBrowser(process.argv[2]);
