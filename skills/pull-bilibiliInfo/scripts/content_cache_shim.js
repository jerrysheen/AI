...
function tryRequireShared() {
  try {
    return require("../../../src/shared/content_cache");
  } catch {
    return null;
  }
}
...
const { getSharedDataDir } = require("./runtime_shim");
...
function updateSourceStats(source) {
  ...
  for (const dateBucket of dayEntries) {
    const index = readJson(getSourceDailyIndexPath(source, dateBucket), null);
    if (!index) {
      continue;
    }
    stats.days.push({
...
function upsertContentCache({ source, category, contentId, publishedAt, record }) {
  ...
  const entryPath = getEntryFilePath({ source, dateBucket, category, contentId });
  const cacheRecord = {
    source,
    category,
