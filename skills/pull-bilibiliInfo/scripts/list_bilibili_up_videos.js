...
async function listUpVideos(urlOrMid, options = {}) {
  ...
  try {
    const scraped = await callCdp(page.webSocketDebuggerUrl, async (send) => {
      ...
      const expression = `(limit) => {
        ...
        for (const anchor of anchors) {
          ...
          // Title anchors on the upload page have readable text; cover anchors mostly contain stats.
    ...
    const filteredVideos = filterVideosByPublishTime(enrichedVideos, publishedAfter, publishedBefore);
    return {
      mid,
      space_url,
      upload_url,
